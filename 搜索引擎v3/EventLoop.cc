...
void EventLoop::handleReadFromTimer(){
    uint64_t one = 1;
    ...
    //将各个cache进行同步，并且进行持久化操作
    CacheManager::getInstance(4);
    ..todo;
}
...
int EventLoop::createTimerFd(){
    ...
    expireTime.it_value.tv_nsec = 0;
    expireTime.it_value.tv_sec = 0;
    expireTime.it_interval.tv_nsec = 0;
    expireTime.it_interval.tv_sec = 0;
    int err = timerfd_settime(fd,0,&expireTime,NULL);
    if(err == -1){
